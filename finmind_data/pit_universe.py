...
the wrong question for a rebalance date: it carries the same 2,159 names on
every session of the window, so screening it at a 2013-06-28 rebalance puts 586
names in that session's universe that were not listed that day. Most of those
are ordinary timing, which dating the list fixes by construction. Two groups
...
- **135 spent part of the window on 興櫃.** The emerging board is not a
  listing, and the registry records only what a code is *now*, so a name
  promoted to TPEx in 2025 reads as TPEx for every session before its
  promotion. It is 90,406 code-sessions, 1.35 % of the panel.
...
**The suspension before a delisting is added back.** For all 179 commons that
delist inside the window, the tape's last session is exactly the name's last
trade in ``ohlcv/``, which is not the delisting — 台一 stopped trading 229 days
before its listing ended. Those sessions are absent from the tape and the
company is still listed, still held, and still owed a terminal value. Presence
alone would drop a name at the moment the delisting-return question starts, so
a span runs to the session before the delisting date. It is 5,538 sessions
across 150 of the 179 — the other 29 last traded on the session before their
exit and have nothing to bridge.

**An interior gap is left to the caller.** 609 codes have at least one session
...
and 48 codes have a gap of 60 sessions or more, up to 8227's nine years, which is
...
def emerging_boundary(raw: pd.DataFrame) -> pd.Series:
    ...
    the query date if it is still there.

    The column is object dtype and 32 rows carry the string ``"None"``, so the
    boundary is parsed rather than compared as text. An unparseable date on an
    emerging row is fatal: it would read as "no 興櫃 phase" and admit the name to
    every session of the window, which is the failure this function exists to
    prevent.

    Takes the registry frame rather than fetching one, because the callers that
    need the boundary have already pulled `taiwan_stock_info` for their own
    reasons — and two pulls minutes apart can disagree about a code that moved
    up from the emerging board between them, which is the one case the boundary
    is consulted about.
    """
    ...
    return emerging.groupby("stock_id")["date"].max()


def _emerging_until() -> tuple[pd.Series, str]:
    """`emerging_boundary` off a fresh pull, with the day it was taken.

    The query date is returned with the boundary and stamped into the artifact —
    ...
    return (emerging_boundary(raw).dt.strftime("%Y-%m-%d"),
...
def build() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for code, quoted in tape[tape["stock_id"].isin(universe)].groupby("stock_id")["date"]:
        ...
        if code in exits.index:
            # 12 of the 179 delist on a day the exchange was shut, so the
