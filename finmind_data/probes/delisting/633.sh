# Session 2026-08-17, transcript call 633.
# (no description recorded)
# Verbatim as run; paths are the session's and it is an archive, not a runnable script.

cd /home/st/research/finance_db && /home/st/miniconda3/bin/python - <<'PY'
import pandas as pd
pd.set_option("display.width", 200); pd.set_option("display.max_colwidth", 90)
lab = pd.read_csv("finmind_data/delisting_labels.csv", dtype={"stock_id": str})
m = lab[lab["label"] == "merger"]
print(len(m), "merger-labelled names\n")
print(m[["stock_id","stock_name","delist_date","drawdown","source"]].to_string(index=False))
PY
